...
-record(state, { module             :: module()
               , max_age = infinity :: millisecs() | infinity
...
-callback init(Args :: list(term())) ->
                  {ok, controller_state()}
                | {ok, controller_state(), {millisecs, millisecs()}
                | {ok, controller_state(), {secs, secs()}}
                | {ok, controller_state(), millisecs()}.
...
init([Module, Args]) ->
    case Module:init(Args) of
        {ok, ControllerState} ->
            ...
            {ok, State};
        {ok, ControllerState, {secs, MaxAgeSecs}} ->
            MaxAge = MaxAgeSecs * 1000,
            State = #state{ module           = Module
                          , max_age          = MaxAge
                          , controller_state = ControllerState},
            {ok, State, MaxAge};
        {ok, ControllerState, {millisecs, MaxAge}} ->
            MaxAge = MaxAgeSecs * 1000,
            State = #state{ module           = Module
                          , max_age          = MaxAge
                          , controller_state = ControllerState},
...
-spec result( {reply, response(), controller_state()}
            ...
            ( {stop, termination_reason(), response(), controller_state()}
            , #state{}) -> {stop, termination_reason(), response(), #state{}};
            
            ( {stop, termination_reason(), controller_state()}
            , #state{}) -> {stop, termination_reason(), #state{}}.
...
result({stop, Reason, Response, ControllerState}, State) ->
    NewState = State#state.controller_state = ControllerState,
    {stop, Reason, Response, NewState};

result({stop, Reason, ControllerState}, State) ->
    NewState = State#state.controller_state = ControllerState,
    {stop, Reason, NewState}.

-spec(sender() | none, replies(), #state{}) -> real_replies().
